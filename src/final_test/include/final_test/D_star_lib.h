...
public:
    ...
    void display(){
        ...
        for (int i=0;i<map_width;i++){
            for(int j=0;j<map_length;j++)
                if(nodes[i][j].get_k()>=999)
                    std::cout<<"& ";
                else if(nodes[i][j].get_is_path())
                    std::cout<<"0 ";
    ...
    int arc(Node * n1, Node * n2){
        if(n1->get_k()>=999 || n2->get_k()>=999)//obstacle
            return 999;
    ...
    void find_path(){
        ...
        for(int i=0;i<8;i++){
            ...
            if(temp!=&nodes[goal_x][goal_y] &&temp->get_k()<999){
        ...
        if(goal_x+1 < map_length){
            if(nodes[goal_x+1][goal_y].get_k()<999)
                nodes[goal_x+1][goal_y].set_k(10);
            if(goal_y+1 < map_width)
                if(nodes[goal_x+1][goal_y+1].get_k()<999)
       ...
       if(goal_x-1>=0){
        ...
        if(goal_y-1>=0)
            if(nodes[goal_x][goal_y-1].get_k()<999)
...
//            display();
        ...
        //find_path--------------------------------------
        Node* temp=&nodes[start_x][start_y];
...
//        display();
    ...
    void set_obstacle(double left, double top, double right, double down){
        if(left>right) std::swap(left,right);
        if(down>top) std::swap(top,down);
      //enlarge the obstacle by 0.5 pixel
//      if(fabs(down-(int)down)>0) down=std::max(0,(int)down-1);
////      else                   down=std::max(0,(int)down-2);
//      if(fabs(top-(int)top)>0) top=std::min(map_width,(int)top+1);
////      else                   top=std::min(map_width,(int)top+2);
//      if(fabs(right-(int)right)>0) right=std::min(map_length,(int)right+1);
